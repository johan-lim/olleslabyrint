        ...
        <div className="title_screen_wrapper">
            ...
            <div className="chooseplaymode">
                <div>
                    ...
                    <button className="startplaying" onClick={props.playGame}>Börja spela</button>
                </div>
                <div>
                    <h2>Pågående spel</h2>
                    <div className="list">
                        {availableRooms.sort((a, b) => b.lastUpdated - a.lastUpdated).map(room => <div className={props.multiPlayerRoom === room.number ? 'roomselected' : ''} key={room.number} onClick={() => props.updateMultiPlayerRoom(room.number)}>Spel startat av: <span className="startedby">{room.startedBy}</span> {room.players} inne <sup>({room.number})</sup></div>)}
