...
export default function LevelEditor(props) {
  ...
  const setcurrentBlockAs = (x, y, newItem) => {
    ...
    const level = currentLevel.map((line, y) =>
      line.map((block, x) => {
  ...
  const createDownload = () => {
    download(
      'level.json',
      JSON.stringify({ blocks: currentLevel }).replace(/10/g, '0')
    );
  };
  ...
  const resetSize = () => {
    const newLevel = new Array(parseInt(height)).fill(
      new Array(parseInt(width)).fill(10)
    );
  ...
  const level = currentLevel.map((line, y) =>
    line.map((block, x) =>
      [20, 21].includes(block) ? (
        block === 20 ? (
          <Gubbe gubbeX={x} gubbeY={y} />
        ) : (
          <Zombie zombieX={x} zombieY={y} gubbeX={x} gubbeY={y} />
        )
      ) : (
        <span onClick={() => setcurrentBlockAs(x, y, currentBlock)}>
          <Block isLevelEditor key={x + y} x={x} y={y} block={block} />
        </span>
      )
    )
  );
    ...
    <div>
      ...
      <div className="level-editor">
        <div className="blocks-picker">
          ...
          {availableBlocks.map((block, i) => (
            <div
              ...
              style={
                currentBlock === block ? { border: '2px solid white' } : {}
              }
            >
        ...
        <div className="blocks-picker">
          ...
          <div
            onClick={() => setCurrentBlock(20)}
            className="editor-block"
            style={currentBlock === 20 ? { border: '2px solid white' } : {}}
          >
            <Gubbe />
          </div>
          <div
            onClick={() => setCurrentBlock(21)}
            className="editor-block"
            style={currentBlock === 21 ? { border: '2px solid white' } : {}}
          >
            <Zombie />
          </div>
          <div
            onClick={() => setCurrentBlock(23)}
            className="editor-block"
            style={currentBlock === 23 ? { border: '2px solid white' } : {}}
          >
            <Zombie isMonster />
          </div>
          <div>
        ...
        <div className="exitorandpreview">
          <div className="preview-level">{level}</div>
          <div>
